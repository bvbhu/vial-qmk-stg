...
// 核心钳位 top ≤ TOPREADING_MAX-8、bottom ≥ BOTTOMREADING_MIN+8；开机实测初始校准读数重写 top。
...
#define ANALOG_DEFAULT_RELEASE_THRESHOLD 1000    /* 释放行程(0..ANALOG_MAX_TRAVEL，与触发点相距 2000 = 最大键程值的一半) */
#define CALIBRATION_THRESHOLD 32               /* 实时校准：偏离校准端点超过该ADC计数才更新 */

// 最大键程值(0=顶部, M=触底)，经 0xF0 caps 发给 Vial。本板 EC 校准端点跨度约 300-400 ADC 计数，
// 但最大键程值取 4000 换取细粒度滑块(行程域与 ADC 计数无关，只是刻度密度)。
// 最大键程值 >255 使行程域升 uint16：配置/记录/读数线格式随之从 12/8/3 变为 16/12/4 字节，
